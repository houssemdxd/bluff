...
*/
    ...
    public function Realgame($id, $player_token, TableRepository $tableRepository, TimeRepository $timeres, RoomRepository $roomRepository, CardsRepository $cardsRepository, PlayerRepository $playerRepository, HandRepository $handRepository, RoundRepository $roundRepository, EntityManagerInterface $em): Response
    {
        ...
        if ($player !== null) {
            $state = $player->getState() ?? 0; // If getState() returns null, assign 0
        } else {
            $state = 0;
        }
...
public function put(
    ...
    try {
        ...
        // Winner declaration logic - REMOVED NESTED TRANSACTION
        $rounds = $roundRepository->findBy(["room" => $room]);
        if (count($rounds) > 0) {
            ...
            if (count($handwinner) == 0) {
                ...
                // Winner player verification - NO NESTED TRANSACTION
        ...
        if ($turnEarned) {
            $room->setGoal($goal);
            $this->publishToWebSocket("/" . $room->getId() . "/goalUpdate", ['goalUpdate' => $goal]);
        }

        // Set the goal for the first time (first round)
        ...
        $this->publishToWebSocket('/' . $room->getId() . '/table', ['cardnumber' => $cardCount]);
...
*/
        ...
        if ($desicion == 1) {
            ...
            //migration to the websocket

            $hubInterface->publish($update);
*/
            $this->publishToWebSocket('/' . $roundplayer->getId() . '/' . 'loserplayer', ['update' => $cardDetails]);
        ...
        # in case he was wrong he will take all the cards and the enemy status updated
            ...
            */
            ...
            $this->publishToWebSocket("/broadcastangel", ['angel' =>$angel]);
...
/*
    ...
    private function publishToWebSocket($topic, $data)
    {
        // Encode the data into JSON with the topic and message structure
        $msg = json_encode(['topic' => $topic, 'data' => $data]);
    
        // Log the message to ensure it's being created correctly
        error_log("Publishing message to WebSocket: " . $msg);
        ...
        $wsUrl = 'ws://127.0.0.1:1700';  // Adjust to your server address
